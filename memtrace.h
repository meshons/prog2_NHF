...
/*ha definialva van, akkor futas kozben lancolt listat epit. Javasolt a
 * hasznalata*/
...
/* A Visual Studio figyelmen kivul hagyja a "throw" deklaraciokat, a gcc
 * -pedantic pedig igenyli.*/
#ifdef _MSC_VER
/* Ha studio */
...
/*Innentol csak a "normal" include eseten kell, kulonben osszezavarja a
 * mukodest*/
...
void *traced_malloc(size_t size, const char *size_txt, int line,
                    const char *file);

#undef calloc
#define calloc(count, size)                                                    \
  TRACEC(traced_calloc)(count, size, #count "," #size, __LINE__, __FILE__)
void *traced_calloc(size_t count, size_t size, const char *size_txt, int line,
                    const char *file);
...
#undef realloc
#define realloc(old, size)                                                     \
  TRACEC(traced_realloc)(old, size, #size, __LINE__, __FILE__)
void *traced_realloc(void *old, size_t size, const char *size_txt, int line,
                     const char *file);
